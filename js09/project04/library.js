...
function init() {
  ...
  function startGame() {
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      currentBlock.firstElementChild.src =
        "block" + scrambledOrder[i - 1] + ".jpg";
...
function swapWithBlank(e) {
  ...
  if (rowNum > 1) {
    blankCheck =
      activeBlock.previousElementSibling.previousElementSibling
  ...
  if (rowNum < 3) {
    blankCheck =
      activeBlock.nextElementSibling.nextElementSibling.nextElementSibling;
    if (blankCheck.id === "block0") {
      swap(activeBlock, blankCheck);
      checkBoard();
      return;
